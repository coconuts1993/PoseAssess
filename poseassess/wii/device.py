...
def _import_hid():
    ...
    This is the ONLY place that imports ``hid`` (lazily, when a real board is searched for), so
    PoseAssess starts and works without hidapi."""
    try:
        import hid
    except ImportError as e:
        raise HidapiUnavailable(HIDAPI_MISSING) from e
    except Exception as e:  # noqa: BLE001 - e.g. OSError: the hidapi DLL / shared library fails to load
        raise HidapiUnavailable(f"hidapi cannot be loaded: {e}") from e
...
class WiiAutoConnect(BalanceBoardHID):
    ...
    Remote) are skipped. When hidapi itself is unusable (missing, shadowed, DLL fails to load)
    ``fatal_error`` is set and the source stops with ``status`` 'error: <reason>'; ``start()``
    tries again.
    ...
    def _run(self) -> None:
        self._retry_at = {}
        self.fatal_error = None  # a restart (start() again) retries, e.g. after installing hidapi
        while not self._stop.is_set():
            dev = self._find_board()
            if dev is None:
                if self.fatal_error:
                    # hidapi is unusable: waiting cannot help. Stop, keeping the reason as the
                    # error ("error: hidapi is not installed (pip install hidapi)")
                    self.error = self.fatal_error
                    return
...
class SimulatedBoard(ForceSource):
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            next_t += period
            if now - next_t > 0.5:  # fell far behind (machine suspended/busy): no catch-up burst
                next_t = now + period


def hidapi_status() -> tuple[bool, str]:
    """``(True, "")`` when hidapi can be used, else ``(False, reason)``. Never raises, so the
    GUI can show why the Balance Board is unavailable and keep running without it. It imports
    ``hid`` when available: call it on demand (e.g. when a Wii page is shown), not at startup."""
    try:
        _import_hid()
    except HidapiUnavailable as e:
        return False, str(e)
    except Exception as e:  # noqa: BLE001 - defensive: never let this break the GUI
        return False, f"hidapi cannot be loaded: {e}"
    return True, ""
