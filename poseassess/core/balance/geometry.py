...
Ported from PoseBoard 3ea6d8a ``poseboard/geometry.py``. PoseAssess differences: the world
frame is the Calib.toml checkerboard frame, whose +Z may point DOWN (see
``calib.world_up_sign``); ``register_board`` handles both (``up_sign``), and UI hints refer to
the page "2. Calibration".
...
from .calib import CameraCalibration, world_up_sign
...
def kabsch(src: np.ndarray, dst: np.ndarray) -> RigidTransform:
    """Rigid transform (det +1) with ``dst ≈ R @ src + t`` (least squares)."""
...
def procrustes_2d(src: np.ndarray, dst: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Planar rotation R (2x2, det +1) and translation t with ``dst ≈ R @ src + t``."""
    src, dst = np.asarray(src, np.float64), np.asarray(dst, np.float64)
    cs, cd = src.mean(0), dst.mean(0)
    H = (src - cs).T @ (dst - cd)
...
def triangulate_point(cams: list[CameraCalibration], pts_px: list[np.ndarray],
    ...
    """Multi-view DLT triangulation (undistorted, normalized coordinates) of ONE point seen at
    ``pts_px[i]`` in ``cams[i]``; returns (3,) world. ValueError with fewer than 2 cameras."""
    if len(cams) < 2:
    ...
    for cam, p, w in zip(cams, pts_px, weights):
        x, y = cam.undistort_normalized(np.asarray(p, np.float64).reshape(1, 2))[0]
...
def backproject_to_plane(cam: CameraCalibration, pts_px: np.ndarray, z: float) -> np.ndarray:
    """Intersect the viewing rays of pixels with the world plane Z = z; (N,3). ValueError when a
    ray misses the plane in front of the camera."""
    ...
    if not np.all(np.isfinite(s)) or np.any(s <= 0):
        raise ValueError("The clicked points do not lie on the floor seen by this camera: check "
                         "the camera extrinsics (2. Calibration; the checkerboard must lie flat "
                         "on the floor)")
    return c + s[:, None] * d_w


def project_to_image(cam: CameraCalibration, pts_w: np.ndarray) -> np.ndarray:
    """Project world points to pixels (N,2); without extrinsics the world is the camera frame."""
...
def reprojection_error(cam: CameraCalibration, pts_w: np.ndarray, pts_px: np.ndarray) -> float:
    """Mean pixel distance between ``project_to_image(cam, pts_w)`` and ``pts_px``."""
...
# A Calib.toml world whose +Z points down is handled by working in the world conjugated by
# FLIP_Z (x' = FLIP_Z @ x, a proper rotation of 180° about X): there +Z' is up.
FLIP_Z = np.diag([1.0, -1.0, -1.0])


def _flipped_camera(cam: CameraCalibration) -> CameraCalibration:
    """``cam`` expressed in the flipped world: x_cam = R @ x + t = (R @ F) @ x' + t."""
    if not cam.has_extrinsics:
        return cam
    return dataclasses.replace(cam, rvec=cv2.Rodrigues(cam.R @ FLIP_Z)[0].ravel(),
                               tvec=np.asarray(cam.tvec, np.float64).ravel().copy())


def _complete(k) -> bool:
    return k is not None and len(k) == 5


# ---------------------------------------------------------- board registration
def register_board(geometry: BoardGeometry, cams: list[CameraCalibration],
                   clicks: list[np.ndarray | None], floor: bool = True,
                   up_sign: int | None = None) -> BoardPose:
    """Board pose in the Calib.toml world from the 5 landmarks (TL, TR, BR, BL, C) clicked in
    each camera (``clicks[i]`` for ``cams[i]``; None or fewer than 5 points = not clicked).

    * Only cameras with extrinsics are used when any clicked camera has them (the others are
      listed in ``notes``). >=2 such cameras: triangulate the 5 points + Kabsch, and the board
      must face every camera (else ValueError(MIRRORED_CLICKS_MSG)). 1 camera:
      ``solve_board_pnp`` (the IPPE branch whose normal is closest to world UP), then camera ->
      world. No clicked camera with extrinsics: PnP in the first camera's frame
      (``world_camera``; the upright-camera prior picks the IPPE branch).
    * Floor fit (``floor``, with extrinsics): if the unconstrained tilt vs world UP is at most
      ``FLOOR_MAX_TILT_DEG``, only x, y and yaw are fitted; the height is the mean triangulated
      height (several cameras) or ``height_mm`` above the checkerboard plane along UP (single
      camera, ``backproject_to_plane``). Tilt > ``TILT_NOTE_DEG`` -> note; tilt >
      ``FLOOR_MAX_TILT_DEG`` -> the 6-DoF fit is kept and a warning is given.
    * ``reproj_error_px`` per name of the cameras used.

    ``up_sign``: +1 when world +Z is up, -1 when world +Z points down (None ->
    ``calib.world_up_sign(cams)``). With -1 the registration runs in the world conjugated by
    ``FLIP_Z`` and is mapped back, so ``BoardPose.up_world`` always points physically up and
    the board frame stays right-handed (x right, y front, z up).
    """
    if up_sign is None:
        up_sign = world_up_sign([c for c in cams if c is not None])
    flip = up_sign < 0 and any(c.has_extrinsics for c, k in zip(cams, clicks) if _complete(k))
    if not flip:
        return _register_board_zup(geometry, cams, clicks, floor)
    pose = _register_board_zup(geometry, [_flipped_camera(c) for c in cams], clicks, floor)
    T = pose.board_to_world
    return dataclasses.replace(pose, board_to_world=RigidTransform(FLIP_Z @ T.R, FLIP_Z @ T.t),
                               warnings=list(pose.warnings), notes=list(pose.notes))


def _register_board_zup(geometry: BoardGeometry, cams: list[CameraCalibration],
                        clicks: list[np.ndarray | None], floor: bool) -> BoardPose:
    """``register_board`` in a world whose +Z is up (PoseBoard 3ea6d8a algorithm)."""
    model = geometry.landmarks()
    usable = [(c, np.asarray(k, np.float64).reshape(5, 2)) for c, k in zip(cams, clicks)
              if _complete(k)]
    if not usable:
        raise ValueError("No camera has all 5 board points (TL, TR, BR, BL, C) clicked")
    ...
    if with_ext:
        ...
        if ignored:
            notes.append(f"Not used: {', '.join(ignored)} (no extrinsics; the world frame is "
                         "the floor checkerboard). Calibrate their extrinsics (2. Calibration) "
                         "to use them.")
        used = with_ext
    else:
        used = usable[:1]
    ...
    if len(used) >= 2:
        ...
        if ambiguous and not (floor and cam.has_extrinsics):
            warnings.append("Two board orientations fit these clicks almost equally well (single "
                            "camera): check the board normal. Calibrating the camera extrinsics "
                            "(2. Calibration) or clicking in a second camera removes this "
                            "ambiguity.")
    ...
    if with_ext:
        ...
        if floor and tilt <= FLOOR_MAX_TILT_DEG:
            ...
            warnings.append(f"The board is tilted {tilt:.0f}° relative to the checkerboard floor. "
                            ...
                            "the board, then redo the clicks. The pose was not fitted to the "
                            "floor.")
...
def solve_board_pnp(cam: CameraCalibration, model: np.ndarray, img_px: np.ndarray,
                    return_ambiguity: bool = False, up_world: np.ndarray | None = None):
    """Board frame -> camera frame from clicked points (planar IPPE + LM refine).

    A planar target has two IPPE solutions and, with noisy clicks, the wrong one can have the
    lower reprojection error. With extrinsics, the solution whose normal is closest to
    ``up_world`` (default world +Z) is used; without, the one whose normal points up in the
    image (upright camera). ValueError when the chosen board faces away from the camera
    (mirrored clicks); RuntimeError when PnP fails. With ``return_ambiguity`` returns
    ``(transform, ambiguous)`` (both IPPE errors within a factor of 2).
    """
    obj = np.asarray(model, np.float64).reshape(-1, 1, 3)
    img = np.asarray(img_px, np.float64).reshape(-1, 1, 2)
    up = np.array([0.0, 0.0, 1.0]) if up_world is None else np.asarray(up_world, np.float64).ravel()
    ...
    if cam.has_extrinsics:
        rvec, tvec, _ = max(sols, key=lambda s: float((cam.R.T @ normal(s)) @ up))
