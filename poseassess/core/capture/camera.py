...
class CameraStream:
    ...
    def request_stop(self) -> None:
        """Ask the capture thread to stop without waiting (``stop`` waits); lets a caller stop
        several cameras in parallel."""
        self._stop.set()

    def stop(self, timeout_s: float | None = None) -> None:
        """Stop capturing, waiting at most ``timeout_s`` (default ``stop_timeout_s``) for the
        capture thread. The capture thread releases the device itself when it leaves
        ``read()``; a read that blocks (e.g. a stalled network stream) is never released under
        its feet."""
        self._stop.set()
        t = self._thread
        if t is not None:
            t.join(timeout=self.stop_timeout_s if timeout_s is None else max(0.0, timeout_s))
            if t.is_alive():
    ...
    def _loop(self, cap: cv2.VideoCapture, stop: threading.Event) -> None:
        ...
        while not stop.is_set():
            ...
            dt = t - last
            last = t
            # the first interval is the open -> first frame latency, not a frame period (a file
            # source returns its first frame at once: the rate would read 1000+ fps for a while)
            if dt > 0 and idx > 1:
